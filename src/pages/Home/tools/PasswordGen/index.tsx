...
/** 核心内容，可在弹框和独立窗口中复用 */
export function PasswordGenContent() {
    ...
    <>
      <div className="pw-row">
        <div className="pw-field pw-field-hal">
          ...
          <input type="number" className="pw-length-input" min={8} max={64} value={length}
            onChange={e => { const v = parseInt(e.target.value, 10); if (!isNaN(v) && v > 0) setLength(v); }} />
        </div>
        <div className="pw-field pw-field-hal">
          <label className="pw-label">生成数量</label>
          <input type="number" className="pw-count-input" min={1} max={999} value={count}
            onChange={e => { const v = parseInt(e.target.value, 10); if (!isNaN(v) && v >= 1 && v <= 999) setCount(v); }} />
        </div>
      </div>
      ...
      <div className="pw-field">
        ...
        <div className="pw-checks">
          <label className="pw-check"><input type="checkbox" checked={upper} onChange={e => setUpper(e.target.checked)} /><span className="pw-check-box" />A-Z</label>
          <label className="pw-check"><input type="checkbox" checked={lower} onChange={e => setLower(e.target.checked)} /><span className="pw-check-box" />a-z</label>
          <label className="pw-check"><input type="checkbox" checked={digits} onChange={e => setDigits(e.target.checked)} /><span className="pw-check-box" />0-9</label>
          <label className="pw-check"><input type="checkbox" checked={symbols} onChange={e => setSymbols(e.target.checked)} /><span className="pw-check-box" />!@#$%</label>
        </div>
      </div>
      ...
      {passwords.length > 0 && (
        <div className="pw-results">
          <div className="pw-results-header">
            ...
            <button className="pw-copy-all-btn" onClick={copyAll}>{copiedAll ? '已复制全部' : '复制全部'}</button>
          </div>
              ...
              {passwords.map((pw, i) => {
                ...
                return (
                  <div key={i} className="pw-item">
                    ...
                    <button className="pw-item-copy" onClick={() => copyOne(pw, i)}>{copiedIdx === i ? '✓' : '复制'}</button>
                  </div>
                );
              })}
            </div>
...
/** 独立窗口版本 */
export function PasswordGenWindow() {
  return (
