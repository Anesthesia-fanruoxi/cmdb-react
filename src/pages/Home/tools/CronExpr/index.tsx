/**
 * Cron 表达式辅助生成工具
 * 可视化选择执行频率，自动生成 cron 表达式并计算最近 5 次执行时间
 */
...
function getNextRuns(cron: string, count: number): Date[] {
  ...
  for (let i = 0; i < 525600 && results.length < count; i++) {
    ...
    if (hasDomRestriction && hasDowRestriction) {
      dayOk = doms.includes(d) || dows.includes(dow);
    } else if (hasDomRestriction) {
      dayOk = doms.includes(d);
    } else if (hasDowRestriction) {
      dayOk = dows.includes(dow);
    } else {
      dayOk = true;
    }
...
function getDescription(freq: FreqType, interval: number, hour: number, minute: number, second: number, weekDays: number[], monthDay: number): string {
  ...
  switch (freq) {
    ...
    case 'minutes': return `每 ${interval} 分钟执行一次`;
    ...
    case 'weekly': {
      const days = weekDays.map(d => '周' + WEEKDAY_LABELS[d]).join('、');
      return `每周${days} ${time}${second > 0 ? `:${String(second).padStart(2, '0')}` : ''} 执行`;
    }
...
function generateCron(freq: FreqType, interval: number, hour: number, minute: number, second: number, weekDays: number[], monthDay: number, cronType: '5' | '6'): string {
  ...
  switch (freq) {
    ...
    case 'minutes': {
      ...
      return cronType === '6' ? `${sec} ${base}` : base;
    }
    case 'hours': {
      const base = `${minute} */${Math.max(1, interval)} * * *`;
      return cronType === '6' ? `${sec} ${base}` : base;
    }
    case 'daily': {
      const base = `${minute} ${hour} * * ${Q}`;
      return cronType === '6' ? `${sec} ${base}` : base;
    }
    case 'weekly': {
      const base = `${minute} ${hour} ${Q} * ${weekDays.sort().join(',')}`;
      return cronType === '6' ? `${sec} ${base}` : base;
    }
    case 'monthly': {
      const base = `${minute} ${hour} ${monthDay} * ${Q}`;
      return cronType === '6' ? `${sec} ${base}` : base;
    }
  }
}
    ...
    <ToolModal visible={visible} title="⏰ Cron 表达式生成" onClose={onClose} size="md">
      ...
      <div className="ce-section">
        ...
        <div className="ce-freq-grid" style={{ gridTemplateColumns: `repeat(${freqOptions.length}, 1fr)` }}>
          {freqOptions.map(opt => (
            <button key={opt.value} className={`ce-freq-btn ${freq === opt.value ? 'active' : ''}`} onClick={() => setFreq(opt.value)}>
              {opt.label}
            </button>
          ))}
        </div>
        ...
        <div className="ce-params">
          ...
          {(freq === 'minutes' || freq === 'hours') && (
            <div className="ce-param-row">
              ...
              {cronType === '6' && (
                <><input type="number" className="ce-num-input ce-num-input--sm" min={0} max={59} value={second} onChange={e => { const v = parseInt(e.target.value, 10); if (!isNaN(v) && v >= 0 && v <= 59) setSecond(v); }} /><span className="ce-param-text">秒触发）</span></>
              )}
            ...
            <>
              <div className="ce-param-row">
                ...
                {WEEKDAY_LABELS.map((label, i) => (
                  <button key={i} className={`ce-weekday-btn ${weekDays.includes(i) ? 'active' : ''}`} onClick={() => toggleWeekDay(i)}>
                    {label}
                  </button>
                ))}
              </div>
