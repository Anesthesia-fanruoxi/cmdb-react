...
const SearchDialog = ({ visible, onClose, editor }: Props) => {
  ...
  // 搜索
  const doSearch = useCallback((text: string, direction: 'next' | 'prev' = 'next') => {
    if (!editor || !text) {
      setTotalCount(0);
      setCurrentIndex(0);
      return;
    ...
    editor.find(text, { backwards: direction === 'prev', wrap: true, caseSensitive, wholeWord: false, regExp: false });
  ...
  useEffect(() => {
    if (visible && searchText) doSearch(searchText);
    else { setTotalCount(0); setCurrentIndex(0); }
  }, [searchText, visible, caseSensitive, doSearch]);
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const dialogWidth = dialogRef.current?.offsetWidth || 380;
    const dialogHeight = dialogRef.current?.offsetHeight || 100;
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') { e.preventDefault(); e.shiftKey ? doSearch(searchText, 'prev') : doSearch(searchText, 'next'); }
  };
  ...
  return (
    <div 
      ref={dialogRef}
      className="search-dialog floating" 
      style={{ left: position.x, top: position.y }}
      onMouseDown={handleMouseDown}
    >
      <div className="search-header">
        <span className="search-title">🔍 查找</span>
      ...
      <div className="search-body">
        <div className="search-input-row">
          ...
          <button className="nav-btn" onClick={() => doSearch(searchText, 'prev')} title="上一个"><ChevronUp size={16} /></button>
          <button className="nav-btn" onClick={() => doSearch(searchText, 'next')} title="下一个"><ChevronDown size={16} /></button>
        </div>
