...
const PageDetailDialog = ({ visible, type, queryId, onClose }: Props) => {
  ...
  return (
    <>
      ...
      <div className="dialog-container page-detail-dialog">
        <div className="dialog-header">
          <h3>{type === 'sql' ? 'SQL' : 'ES'}详情</h3>
        </div>
          ...
          {loading ? (
                  ...
                  <table className="ops-table">
                    ...
                    <tbody>
                      {operations.map((op, idx) => (
                        <tr key={idx}>
                          <td><span className={`tag ${op.Operation === 'export' ? 'warning' : 'info'}`}>{op.Operation === 'export' ? '导出' : op.Operation === 'page' ? '翻页' : '查询'}</span></td>
                          <td>{op.nick_name}</td>
            ...
            <>
              <div className="detail-grid">
                ...
                <div className="detail-item"><span className="label">客户端IP</span><span className="value">{data?.client_ip || '-'}</span></div>
                <div className="detail-item"><span className="label">地区</span><span className="value">{formatLocation(data)}</span></div>
                <div className="detail-item"><span className="label">项目名称</span><span className="value">{data?.project_name || '-'}</span></div>
                <div className="detail-item"><span className="label">视图名称</span><span className="value">{data?.view_name || '-'}</span></div>
                <div className="detail-item"><span className="label">页码</span><span className="value">{data?.page || '-'}</span></div>
                <div className="detail-item"><span className="label">索引模式</span><span className="value">{data?.q_index_pattern || '-'}</span></div>
                <div className="detail-item"><span className="label">时间字段</span><span className="value">{data?.q_time_field || '-'}</span></div>
                <div className="detail-item"><span className="label">关键词</span><span className="value">{data?.keyword || '-'}</span></div>
                <div className="detail-item"><span className="label">查询ID</span><span className="value">{data?.query_id || '-'}</span></div>
                <div className="detail-item"><span className="label">查询耗时</span><span className="value"><span className={`tag ${data?.query_time_ms > 1000 ? 'warning' : 'success'}`}>{data?.query_time_ms || 0}ms</span></span></div>
                <div className="detail-item"><span className="label">文档数量</span><span className="value"><span className="tag info">{data?.doc_count || 0}</span></span></div>
                <div className="detail-item"><span className="label">状态码</span><span className="value"><span className={`tag ${data?.response_code === 200 ? 'success' : 'danger'}`}>{data?.response_code}</span></span></div>
                <div className="detail-item"><span className="label">操作时间</span><span className="value">{formatDateTime(data?.search_time)}</span></div>
              </div>
              {operations.length > 0 && (
                <div className="operations-section">
                  <h4>关联操作记录 ({operations.length}条)</h4>
                  <table className="ops-table">
                    ...
                    <tbody>
                      {operations.map((op, idx) => (
                        <tr key={idx}>
                          <td><span className={`tag ${op.operation === 'export' ? 'warning' : 'info'}`}>{op.operation === 'export' ? '导出' : op.operation === 'page' ? '翻页' : op.operation}</span></td>
                          <td>{op.nick_name}</td>
      ...
      <style>{`
        .page-detail-dialog { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 800px; max-width: 90%; max-height: 80vh; background: var(--bg-color); border-radius: 8px; z-index: 1101; display: flex; flex-direction: column; }
        ...
        .operations-section h4 { font-size: 14px; font-weight: 500; margin-bottom: 12px; padding-top: 12px; border-top: 1px solid var(--border-color); }
        .ops-table { width: 100%; border-collapse: collapse; font-size: 13px; }
        .ops-table th, .ops-table td { padding: 10px 12px; text-align: left; border: 1px solid var(--border-color); }
        .ops-table th { background: var(--bg-secondary); font-weight: 500; }
