...
const SearchTabs = ({
  ...
  const handleTabMouseDown = useCallback((e: React.MouseEvent, tabId: string) => {
    ...
    const onMouseMove = (ev: MouseEvent) => {
      ...
      if (st.mode === 'scroll') {
        ...
        // 占据相邻标签 80% 宽度时才交换位置
        const container = tabsContainerRef.current;
        if (!container || !onTabsReorder) return;
        ...
        const commit = (arr: TabData[]) => onTabsReorder(arr.map(t => ({ id: t.id, name: t.name })));
  ...
  }, [onTabChange, onDuplicateTab, onTabsReorder]);
  ...
  return (
    <div className="tabs-header">
        ...
        {tabs.map(tab => {
          ...
          return (
            <div
              ...
              onMouseDown={(e) => { if (!draggedId) handleTabMouseDown(e, tab.id); }}
      ...
      <span className="tabs-tip">双击复制 | 下拉排序 | 右键菜单</span>
      ...
      {contextMenu && (
        ...
        >
          <div className="tab-context-item" onClick={() => { onDuplicateTab(contextMenu.tabId); setContextMenu(null); }}>
            📋 复制标签页
          </div>
          {onTabDetach && (
          ...
          {tabs.length > 1 && (
            <div className="tab-context-item danger" onClick={() => { onCloseTab(contextMenu.tabId); setContextMenu(null); }}>
              🗑️ 删除
            </div>
          )}
        </div>
