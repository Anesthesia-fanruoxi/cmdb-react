...
static HARDWARE_FINGERPRINT: OnceLock<Result<String, String>> = OnceLock::new();
static DEVICE_KEY: OnceLock<Result<Vec<u8>, String>> = OnceLock::new();
...
fn compute_hardware_fingerprint() -> Result<String, String> {
...
/// 获取硬件指纹（更稳定，基于硬件信息）
/// Windows 下会调用 wmic，结果只算一次并缓存。
pub fn get_hardware_fingerprint() -> Result<String, String> {
    HARDWARE_FINGERPRINT
        .get_or_init(compute_hardware_fingerprint)
        .clone()
}

fn compute_device_key() -> Result<Vec<u8>, String> {
...
pub fn get_device_key() -> Result<Vec<u8>, String> {
    DEVICE_KEY.get_or_init(compute_device_key).clone()
}
